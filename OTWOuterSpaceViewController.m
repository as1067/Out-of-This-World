...
{
    /* The prepareForSegue method is called right before the viewController transition occurs. Notice that we do introspection to ensure that the Segue is being triggered by the UITableViewCell. We then confirm that the destination ViewController is the OTWSpaceImageViewController. Finally, we create a variable named nextViewController that points to our destination ViewController. Determine the indexPath of the selected cell and use that indexPath to access a OTWObject in our planet array. Finally set the property spaceobject of the variable nextViewController equal to the selected object. */
    ...
    if ([sender isKindOfClass:[UITableViewCell class]])
    {
        if ([segue.destinationViewController isKindOfClass:[OTWOuterSpaceViewController class]])
        {
            ...
            OTWObject *selectedObject = self.planets[path.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    /* Access the OTWObject from our planets array. Use the OTWObject's properties to update the cell's properties.*/
